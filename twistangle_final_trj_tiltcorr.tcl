...
##########################################
set fp [open "twist_final_trj.dat" w]
...
for {set k 0} {$k < $nf} {incr k} {
	...
	foreach chain $chainlist {
		set atom_gr1_cent [measure center [atomselect top "segname ${chain}2 and resid 47 to 66 and backbone and noh"]]
		set atom_gr2_cent [measure center [atomselect top "segname ${chain}1 and resid 47 to 66 and backbone and noh"]]
...
#	puts "2nd Thanks"
	$sel2 delete
	$all delete
#	puts "vectors $norm_vectors"
#	puts "no. of vectors [llength $norm_vectors]"
##########################################
...
######## Calculating the angle b/w two subsequent vectors
	...
	for {set j 0} {$j < $gap} {incr j} {
		set ang1 [expr acos(max(-1.0,min(1.0,([vecdot [lindex $norm_vectors $j] [lindex $norm_vectors [expr $j+1]]]))))]
                set ang1deg [expr $ang1 * 57.3]
